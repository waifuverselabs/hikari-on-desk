const { app, BrowserWindow, screen, ipcMain, globalShortcut, nativeTheme, dialog, shell, nativeImage, powerSaveBlocker, clipboard, powerMonitor } = require("electron");
...
// If lenient fell back to the default theme OR the variant fell back to "default",
...
let _requestedThemeId = _settingsController.get("theme") || themeLoader.DEFAULT_THEME_ID;
...
if (codexPetMain.summaryHasActiveOrphan(_startupCodexPetSyncSummary, _requestedThemeId)) {
  ...
  _requestedThemeId = themeLoader.DEFAULT_THEME_ID;
...
// ── Ambient desktop-activity source (hikari only) ──
// Reacts to the user's own desktop activity (typing / reading a document /
// audio playing) when no agent session is active. Detection is permission-free;
// the state machine arbitrates priority (agents win, ambient fills gaps).
const _ambientCtx = {
  get enabled() { return _settingsController.get("ambientActivity") === true; },
  get isHikari() { const t = getActiveTheme(); return !!(t && t._id === "hikari"); },
  get doNotDisturb() { return doNotDisturb; },
  get miniMode() { return _mini.getMiniMode(); },
  get dragLocked() { return petWindowRuntime.isDragLocked(); },
  get menuOpen() { return menuOpen; },
...
const _ambient = require("./ambient-activity")(_ambientCtx);
...
const _menuCtx = {
  ...
  getActiveThemeId: () => themeRuntime.getActiveThemeId(themeLoader.DEFAULT_THEME_ID),
