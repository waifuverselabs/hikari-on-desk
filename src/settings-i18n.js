...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      rowAmbientActivity: "Ambient desktop activity",
      rowAmbientActivityDesc: "When no agent is running, the pet reacts to your own desktop — typing, an open document, or audio playing. Hikari theme only.",
      ...
      themeImportUserThemeZip: "Import Hikari theme package (.zip)",
      themeImportUserThemeZipHint: "Hikari theme zip: root or one top-level folder must contain theme.json. Put SVG, GIF, APNG, PNG, WebP, JPG, or JPEG assets in assets/. Use Codex Pet import for Codex Pet packages.",
    ...
    zh: {
      ...
      rowAmbientActivity: "桌面活动感知",
      rowAmbientActivityDesc: "没有 agent 运行时，桌宠会根据你的桌面活动做出反应：打字、打开文档或正在播放音频。仅限 Hikari 主题。",
      ...
      themeImportUserThemeZip: "导入 Hikari 主题包（.zip）",
      themeImportUserThemeZipHint: "Hikari 主题 zip：根目录或一个顶层文件夹内必须包含 theme.json；素材放在 assets/，可使用 SVG、GIF、APNG、PNG、WebP、JPG、JPEG。Codex Pet 包请用上方入口。",
    ...
    "zh-TW": {
      ...
      rowAmbientActivity: "桌面活動感知",
      rowAmbientActivityDesc: "沒有 agent 執行時，桌寵會依你的桌面活動做出反應：打字、開啟文件或正在播放音訊。僅限 Hikari 主題。",
      ...
      themeImportUserThemeZip: "匯入 Hikari 主題套件（.zip）",
      themeImportUserThemeZipHint: "Hikari 主題 zip：根目錄或單一頂層資料夾裡要有 theme.json；素材放在 assets/，支援 SVG、GIF、APNG、PNG、WebP、JPG、JPEG。Codex Pet 套件請改用上面的入口。",
    ...
    ko: {
      ...
      rowAmbientActivity: "데스크톱 활동 반응",
      rowAmbientActivityDesc: "에이전트가 실행 중이 아닐 때, 입력·문서 열기·오디오 재생 등 사용자의 데스크톱 활동에 펫이 반응합니다. Hikari 테마 전용.",
      ...
      themeImportUserThemeZip: "Hikari 테마 패키지 가져오기 (.zip)",
      themeImportUserThemeZipHint: "Hikari 테마 zip: 루트 또는 하나의 최상위 폴더에 theme.json이 있어야 합니다. SVG, GIF, APNG, PNG, WebP, JPG, JPEG 파일은 assets/에 넣으세요. Codex Pet 패키지는 위의 가져오기를 사용하세요.",
    ...
    ja: {
      ...
      rowAmbientActivity: "デスクトップ活動に反応",
      rowAmbientActivityDesc: "エージェントが動いていないときに、入力・ドキュメント表示・音声再生など自分のデスクトップ操作にペットが反応します。Hikari テーマ専用。",
      ...
      themeImportUserThemeZip: "Hikari テーマパッケージをインポート（.zip）",
      themeImportUserThemeZipHint: "Hikari テーマ zip: ルートまたは 1 つのトップフォルダに theme.json が必要です。SVG、GIF、APNG、PNG、WebP、JPG、JPEG は assets/ に入れてください。Codex Pet パッケージは上の入口を使います。",
