...
// Milliseconds since the last real user input (keyboard OR mouse), via the
// permission-free Electron API. Used so the sleep timer counts keyboard
// activity too — otherwise she dozes off while you're typing (no mouse move).
function getSystemIdleMs() {
  if (powerMonitor && typeof powerMonitor.getSystemIdleTime === "function") {
    try { return powerMonitor.getSystemIdleTime() * 1000; } catch { /* ignore */ }
  }
  return null;
}
...
function runMainTickOnce() {
    ...
    // Normal idle: inactivity detection (keyboard + mouse) + sleep sequence
    if (idleNow) {
      // Keyboard counts as activity too. getSystemIdleTime() resets on ANY
      // input, so if the last real input is more recent than our idle anchor,
      // the user is active — treat it exactly like a mouse move so she stays
      // awake while typing and only sleeps after true inactivity.
      let active = moved;
      if (!active) {
        const sysIdleMs = getSystemIdleMs();
        if (sysIdleMs != null && Date.now() - sysIdleMs > mouseStillSince) active = true;
      }
      if (active) {
        mouseStillSince = Date.now();
        hasTriggeredYawn = false;
