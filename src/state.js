...
let screen, powerMonitor;
try { ({ screen, powerMonitor } = require("electron")); } catch { screen = null; powerMonitor = null; }

// ms since the last real user input (keyboard OR mouse); null if unavailable.
function systemIdleMs() {
  if (powerMonitor && typeof powerMonitor.getSystemIdleTime === "function") {
    try { return powerMonitor.getSystemIdleTime() * 1000; } catch { return null; }
  }
  return null;
}
const {
...
// agent session is active — see resolveDisplayState / resolveDisplayStateFromSessions.
  ...
  if (remaining > 0) {
    ...
    pendingTimer = setTimeout(() => {
      ...
      if (ONESHOT_STATES.has(queued)) {
        ...
        // Route through applyResolvedDisplayState so the ambient "me first"
        // override is honored after the min-display window elapses.
        applyResolvedDisplayState();
      }
    }, remaining);
  } else {
...
function applyResolvedDisplayState() {
  // "Me first": your own active desktop activity (typing / audio) overrides the
  // agent. Apply it with the theme's BASE state art (resolveVisualBinding), not
  // getSvgOverride — the latter is session-tier aware and would show the agent's
  // multi-session "building" art for your own typing.
  const ambOverride = resolveAmbientOverride();
  if (ambOverride) {
    const svg = resolveVisualBinding(ambOverride);
    // Dedupe so repeated agent events (while you keep typing / audio keeps
    // playing) don't restart the ambient animation every tick.
    if (ambOverride !== currentState || svg !== currentSvg) {
      applyState(ambOverride, svg, {});
    }
    return;
  }
...
function startWakePoll() {
  ...
  wakePollTimer = setInterval(() => {
    ...
    // Wake on ANY input, not just the mouse: a keystroke should wake her too.
    const idleMs = systemIdleMs();
    const keyboardActive = idleMs != null && idleMs < 700;

    if (moved || keyboardActive) {
      stopWakePoll();
      wakeFromDoze();
      return;
    }

    // Deep sleep on TRUE inactivity (keyboard + mouse), not just mouse stillness.
    const inactiveMs = idleMs != null ? idleMs : Date.now() - ctx.mouseStillSince;
    if (currentState === "dozing" && inactiveMs >= DEEP_SLEEP_TIMEOUT) {
      stopWakePoll();
...
// "Me first, agent fills gaps": your own desktop activity outranks the agent.
//  · ACTIVE ambient (you're typing → "working", or audio is playing → "music")
//    wins over every normal agent work state; it yields ONLY to things you must
//    act on — permission/notification (7) and error (8).
//  · PASSIVE ambient (a document open with no typing → "thinking") only fills
//    the gap when the agent is idle.
// DND suppresses ambient entirely. Returns the ambient state to show, or null.
const ACTIVE_AMBIENT = new Set(["working", "music"]);
const ACTIVE_AMBIENT_PRIORITY = 6.5;

function resolveAmbientOverride() {
  const amb = ctx.doNotDisturb ? null : ambientState;
  if (!amb) return null;
  const agentResolved = resolveDisplayState();
  if (ACTIVE_AMBIENT.has(amb)
      && getStatePriority(agentResolved, STATE_PRIORITY) < ACTIVE_AMBIENT_PRIORITY) {
    return amb;
  }
  if (agentResolved === "idle") return amb;
  return null;
}

// Final "what should the pet show right now" decision (ambient + agent),
// independent of min-display debouncing. Exposed for tests / introspection.
function getDisplayTarget() {
  return resolveAmbientOverride() || resolveDisplayState();
}

// Ambient desktop-activity source entry point (called by src/ambient-activity.js).
function setAmbientState(next) {
  const normalized = next || null;
  if (normalized === ambientState) return;
  ambientState = normalized;
  applyResolvedDisplayState();
}
...
return {
  ...
  setAmbientState, getDisplayTarget,
  shouldDropForDnd,
